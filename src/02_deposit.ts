...
import { increaseLiquidityInstructions, setNativeMintWrappingStrategy } from "@orca-so/whirlpools";
...
async function main() {
  ...
  const { quote: depositQuote, instructions: depositIxs } = await increaseLiquidityInstructions(
    kitRpc,
    position.data.positionMint,
    { tokenA: 100_000n }, // quote based on tokenA amount
    300, // slippage bps (3%)
    createNoopSigner(vaultPda),
  );

  console.log("deposit quote:", depositQuote);
